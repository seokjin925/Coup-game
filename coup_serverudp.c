   ...
   int main(int argc, char *argv[])
   {
    ...
    for(int i=0;i<4;i++){
      ...
      while(1){
            ...
            memset(BUFF, '\0', sizeof(BUFF));                           // 1번 접속자 처리
        ...
        // 클라이언트에게 현재 플레이어의 직업 정보 전송
                ...
                if(strstr(ch_buff,"challenge") != NULL){
                    if(strstr(BUFF, "1") != NULL){
                        if(assassin1 == 2)
                            ...
                            challenge(player,player,0,user_index,1);
                    }
                    if(strstr(BUFF, "2") != NULL){
                        if(assassin1 == 2)
         ...
         for(int i =0;i<4;i++)
         {
            check += dead_player_check[i];
         }
         
         if(check ==3)
            {
               for(int i=0;i<4;i++)
                  if(dead_player_check[i] == 0){
                     memset(BUFF, '\0', sizeof(BUFF));   
                     sprintf(BUFF, "최종 승리!)\n");
                     sendto(server_socket, BUFF,100,0,(struct sockaddr*)&client_addr[i], clnt_adr_sz);
...
void assassin(struct user_node *player1, struct user_node *player2, int index1)
{
   ...
   sprintf(MSG,"암살 대상 플레이어를 골라주세요! (player*)");
...
void ambassador(struct user_node *player1, int index1){
      ...
      write(player1[index1].user_sockfd,MSG,strlen(MSG)); // 현재 턴인 유저
      while(1){
         memset(MSG,'\0',sizeof(MSG));
